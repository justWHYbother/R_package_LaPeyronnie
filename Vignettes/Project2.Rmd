...
Butterfly<-read.csv("/cloud/project/Data/Butterfly_data.csv")
Phenology
Butterfly
...
```{r, eval=FALSE}
ggplot(Butterfly, aes(x = SpringTemp, y = Day)) + geom_point(size = 0.5)
ggplot(Butterfly, aes(x = SummerTemp, y = Day)) + geom_point(size = 0.5)
ggplot(Butterfly, aes(x = Year, y = Day)) + geom_point(size = 0.5)
#Personally, I think the temps (summer and spring) will be the most relevant because they will show what temp each species prefers emerge at. However, I do not think there will be a strong correlation relationship
```

5) Is there a significant linear relationship between any predictors and the response? (10 pts)

```{r}
modelSpring <- lm(Day ~ SpringTemp, data = Butterfly)
ggplot(Butterfly, aes(x = SpringTemp, y = Day)) + geom_point(size = 0.5) + geom_smooth(method = "lm")
modelSummer <- lm(Day ~ SummerTemp, data = Butterfly)
ggplot(Butterfly, aes(x = SummerTemp, y = Day)) + geom_point(size = 0.5) + geom_smooth(method = "lm")
modelDay<- lm(Day ~ Year, data = Butterfly)
ggplot(Butterfly, aes(x = Year, y = Day)) + geom_point(size = 0.5) + geom_smooth(method = "lm")
```

```
There are no significant linear relationships that I can see. Personally it looks very messy and chaotic.
```
...
```{r, eval=FALSE}
ggplot(Butterfly, aes(x = SummerTemp, y = Day)) + geom_point(size = 0.5) + geom_smooth(method = "lm") + facet_wrap(facets = vars(ButterflySpecies))
ggplot(Butterfly, aes(x = SpringTemp, y = Day)) + geom_point(size = 0.5) + geom_smooth(method = "lm") + facet_wrap(facets = vars(ButterflySpecies))
```
...
```{r}
ButterflyTotal<-(merge(Butterfly,Phenology))
ggplot(ButterflyTotal, aes(x = SummerTemp, y = Day)) + geom_point(size = 0.5) + geom_smooth(method = "lm") + facet_wrap(facets = vars(ButterflySpecies))
ggplot(ButterflyTotal, aes(x = SpringTemp, y = Day)) + geom_point(size = 0.5) + geom_smooth(method = "lm") + facet_wrap(facets = vars(ButterflySpecies))
ggplot(ButterflyTotal, aes(x = AnnualTemp, y = Day)) + geom_point(size = 0.5) + geom_smooth(method = "lm") + facet_wrap(facets = vars(ButterflySpecies))
...
```{r}
#I'd say that all the temperature relationships (summer, spring, annual) to day of emergence are all pretty weak. When we look at them by species based of the best fit lines and their lm functions the spring temperature had the best of the "fit" lines with the least standard error. This relates back to Kharouba and Vellend's hypotheses becaues they were trying to see if aboitic factors like temperature are condusive to the emergency of the butterflys out side of the flowering of the plants they feed off.
```
